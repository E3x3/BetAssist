    ...
    def findGoodBets(self, betData, overBetThreshold, underBetThreshold, numGames):
        ...
        for i in range(len(betData)):
            ...
            if overageAmount > overBetThreshold:
                betInfo = list(entry)
                betInfo.append(round(overageAmount, 3))
                betInfo.append("TAKE THE OVER ")
                goodBetList.append(betInfo)

            if underageAmount > underBetThreshold:
                betInfo = list(entry)
                betInfo.append(round(underageAmount, 3))
                betInfo.append("TAKE THE UNDER ")
                goodBetList.append(betInfo)
...
if __name__ == '__main__':
    ...
    betAssist = BetAssist()
    print(betAssist.findGoodBets(betData, 0.1, 0.2, 5))
...
'''
               (1629027, 37.5, 'pts_asts_rebs'), (202710, 40.5, 'pts_asts_rebs'), (1627749, 29.5, 'pts_asts_rebs'), (1628389, 32.5, 'pts_asts_rebs'),
               (203991, 20.5, 'pts_asts_rebs'), (1629639, 29.5, 'pts_asts_rebs'), (201942, 32.5, 'pts_asts_rebs'), (1627832, 31.5, 'pts_asts_rebs'),
               (203897, 34.5, 'pts_asts_rebs'), (1627783, 36.5, 'pts_asts_rebs')]
'''
